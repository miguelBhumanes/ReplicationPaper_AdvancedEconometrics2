...
# =========================
# 17) Diagnostics for projection dataset (independent)
# =========================
wedge_stat_cols = ["wedge1_stat", "wedge2_stat", "wedge3_stat", "wedge4_stat", "wedge5_stat"]

u_cols = [c for c in resid_df.columns if c.startswith("u_")]
print("n_u_cols:", len(u_cols))
print("first_u_cols:", u_cols[:10])

print("\nresid_df cols head:", resid_df.columns[:10].tolist())
print("wedges_df cols head:", wedges_df.columns[:10].tolist())

wstat = wedges_df[wedge_stat_cols].copy()
wstat = wstat.reset_index().rename(columns={"index": DATE_COL})
wstat[DATE_COL] = pd.to_datetime(wstat[DATE_COL])

resid_tmp = resid_df[[DATE_COL] + u_cols].copy()
resid_tmp[DATE_COL] = pd.to_datetime(resid_tmp[DATE_COL])

print("\nDATE ranges:")
print("wedges:", wstat[DATE_COL].min(), "->", wstat[DATE_COL].max(), "n=", wstat.shape[0])
print("resid :", resid_tmp[DATE_COL].min(), "->", resid_tmp[DATE_COL].max(), "n=", resid_tmp.shape[0])

common_dates = np.intersect1d(wstat[DATE_COL].values.astype("datetime64[ns]"),
                             resid_tmp[DATE_COL].values.astype("datetime64[ns]"))
print("\ncommon dates:", common_dates.size)

merged_raw = pd.merge(wstat, resid_tmp, on=DATE_COL, how="inner")
print("merged_raw shape:", merged_raw.shape)

na_by_col = merged_raw.isna().sum().sort_values(ascending=False)
print("\nTop 15 columns by NaN count after merge (before dropna):")
print(na_by_col.head(15))

merged_drop_all = merged_raw.dropna()
print("\nmerged_drop_all shape (dropna on all columns):", merged_drop_all.shape)

for wcol in wedge_stat_cols:
    keep_cols = [wcol] + u_cols
    sub = merged_raw[[DATE_COL] + keep_cols]
    sub_drop = sub.dropna()
    print(f"\n{wcol}:")
    print("  merged rows:", sub.shape[0])
    print("  rows after dropna on [wedge + u_cols]:", sub_drop.shape[0])
    if sub_drop.shape[0] > 0:
        X = sub_drop[u_cols].to_numpy(dtype=float)
        y = sub_drop[wcol].to_numpy(dtype=float)
        print("  X shape:", X.shape, "y shape:", y.shape)
        print("  y var:", float(np.var(y)), "X var min:", float(np.min(np.var(X, axis=0))))
    else:
        print("  -> empty after dropna (date mismatch or NaNs wiping everything)")

if len(u_cols) > 0:
    uniq_u = resid_tmp[DATE_COL].nunique()
    print("\nUnique dates in resid_tmp:", uniq_u, "duplicates:", resid_tmp.shape[0] - uniq_u)
    if uniq_u != resid_tmp.shape[0]:
        dup_dates = resid_tmp[DATE_COL][resid_tmp[DATE_COL].duplicated()].head(10)
        print("Example duplicated dates in resid_tmp:", dup_dates.tolist())

uniq_w = wstat[DATE_COL].nunique()
print("Unique dates in wstat:", uniq_w, "duplicates:", wstat.shape[0] - uniq_w)
if uniq_w != wstat.shape[0]:
    dup_dates = wstat[DATE_COL][wstat[DATE_COL].duplicated()].head(10)
    print("Example duplicated dates in wstat:", dup_dates.tolist())
...
# check quarterly frequency + "first day of last month" convention
d = pd.to_datetime(factors_df[DATE_COL])

print("unique month-days:", pd.Series(d.dt.day.unique()).sort_values().tolist())
print("unique months:", pd.Series(d.dt.month.unique()).sort_values().tolist())

dq = d.dt.to_period("Q")
print("one obs per quarter:", dq.nunique() == len(dq))

d_qend = dq.dt.to_timestamp("Q")
print("all equal to quarter-end after conversion:", (d_qend == d_qend).all())
print("example mapping:", pd.DataFrame({"orig": d.head(8).values, "q_end": d_qend.head(8).values}))
